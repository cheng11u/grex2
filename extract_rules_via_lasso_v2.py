...
if __name__ == "__main__":
    ...
    extracted_rules['pattern_q'] = conclusion or ""
    if conclusion_meta:
        extracted_rules['pattern_q'] += ",".join([f"{k}={v}" for k, v in conclusion_meta.items()])
    ...
    for j, alpha in enumerate(alphas):
        ...
        for name, (value, idx) in feature_set.feature_weights(model.coef_[0]).items():
            if name not in all_rules:
                ...
                coverage_notq = n_pattern_negative_occurence / n_matched

                if n_pattern_positive_occurence / n_matched > int(y.sum()) / len(data):
                    decision = 'yes'
                    ratio = coverage_q / coverage_notq if coverage_notq != 0 else 0
                    coverage = coverage_q * 100
                    presicion = (n_pattern_positive_occurence / n_matched) * 100
                else:
                    decision = 'no'
                    ratio = coverage_notq / coverage_q if coverage_q != 0  else 0
                    coverage = coverage_notq * 100
                    presicion = (n_pattern_negative_occurence / n_matched) * 100

                ordered_rules.append({
                    "pattern": name,
                    "p_occurrences": int(n_matched),
                    "p-q_occurrences": int(n_pattern_positive_occurence),
                    "p-notq_occurrences": int(n_pattern_negative_occurence),
                    "ratio": ratio if ratio != 0 else float(1e6),
